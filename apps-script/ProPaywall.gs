...
function startProCheckout_(params) {
  ...
  const session = stripeRequest_('/v1/checkout/sessions', 'post', {
    ...
    success_url:webAppUrl+'?action=pro-return&session_id={CHECKOUT_SESSION_ID}', cancel_url:webAppUrl+'?action=pro-cancel',
    'metadata[uid]':uid, 'metadata[purpose]':'v2_lifetime_pro', 'line_items[0][quantity]':'1',
    'line_items[0][price_data][currency]':PRO_CONFIG.CURRENCY.toLowerCase(), 'line_items[0][price_data][unit_amount]':String(PRO_CONFIG.PRICE_PENCE),
...
function finishProCheckout_(params) {
  const sessionId=clean_(params.session_id), props=PropertiesService.getScriptProperties(), key='PRO_SESSION_'+sessionId;
  ...
  pending.status='COMPLETED';pending.paidAt=Date.now();props.setProperty(key,JSON.stringify(pending));
...
function grantLifetimePro_(uid,sessionId){
  ...
  const now=new Date().toISOString(),payload={fields:{plan:{stringValue:'pro'},proAccess:{stringValue:'lifetime'},proPurchasedAt:{timestampValue:now},proStripeSessionId:{stringValue:sessionId},updatedAt:{timestampValue:now}}};
