const CONFIG = {
  ...
  INTERNAL_EMAIL: 'info@volumetwo.co.uk', SITE_URL: 'https://blacksurvivalgear.github.io/VOLUMETWO/'
};
...
// Shared Stripe transport retained for the separate Lifetime Pro Business Tools checkout.
...
function bookSlot_(body) {
  ['start','end','name','email'].forEach(key=>{if(!body[key]) throw new Error('Missing '+key);});
  ...
  const challenges=cleanList_(body.challenge).join(', '), help=cleanList_(body.help).join(', ');
  const description=['Volume Two Discovery Call','Booking ID: '+bookingId,'Name: '+clean_(body.name),'Business: '+clean_(body.business),'Email: '+clean_(body.email),'Phone: '+clean_(body.phone),'Website: '+clean_(body.website),'Challenge: '+challenges,'Help wanted: '+help,'Customer/service value: '+clean_(body.value),'Urgency: '+clean_(body.urgency),'Payment: Not required'].join('\n');
