import { getAuth, onAuthStateChanged, signInWithEmailAndPassword, signInWithPopup, createUserWithEmailAndPassword, getAdditionalUserInfo, updateProfile, GoogleAuthProvider, signOut } from "https://www.gstatic.com/firebasejs/12.18.0/firebase-auth.js";
...
const isMemberPage = window.location.pathname.endsWith('/dashboard.html') || window.location.pathname.endsWith('/business-tools.html') || window.location.pathname.endsWith('/business-profile.html') || window.location.pathname.endsWith('/community.html');
...
const VALID_PLANS = Object.keys(PLAN_LABELS);
...
async function ensureUserProfile(user) {
  ...
  const role = isSuperadmin ? 'superadmin' : (existing && VALID_ROLES.includes(existing.role) ? existing.role : 'member');
  const plan = existing && VALID_PLANS.includes(existing.plan) ? existing.plan : 'free';
  const suspended = existing?.suspended === true;
  ...
  if (!existing) {
    ...
    return { role, plan, suspended: false, accountStatus: 'active' };
  }
  ...
  return { role, plan, suspended, accountStatus };
}

async function resolveProfile(user) {
  try { return await ensureUserProfile(user); }
  catch (error) { console.error('Unable to load V2 user profile:', error); return { role: 'member', plan: 'free', suspended: false, accountStatus: 'active' }; }
}

function setRoleUI(role, plan = 'free', accountStatus = 'active') {
  document.querySelectorAll('[data-auth-role]').forEach((el) => { el.textContent = ROLE_LABELS[role] || 'Member'; });
  document.querySelectorAll('[data-auth-plan]').forEach((el) => { el.textContent = PLAN_LABELS[plan] || 'Free'; });
  document.querySelectorAll('[data-account-status]').forEach((el) => { el.textContent = accountStatus === 'suspended' ? 'Suspended' : 'Active'; });
...
if (isSignInPage) {
  ...
  const showView = (view) => { const creating = view === 'create'; signInView?.toggleAttribute('hidden', creating); createAccountView?.toggleAttribute('hidden', !creating); const title = document.querySelector('#auth-title'); if (title) title.textContent = creating ? 'Create your account.' : 'Welcome back.'; setStatus(''); (creating ? document.querySelector('#new-name') : document.querySelector('#email'))?.focus(); };
  ...
  createForm?.addEventListener('submit', async (event) => { event.preventDefault(); const displayName = document.querySelector('#new-name').value.trim(); const email = document.querySelector('#new-email').value.trim(); const password = document.querySelector('#new-password').value; const confirmPassword = document.querySelector('#new-password-confirm').value; const submitButton = createForm.querySelector('button[type="submit"]'); if (!displayName) { setStatus('Please enter your name.', 'error'); return; } if (password.length < 6) { setStatus('Please use a password with at least 6 characters.', 'error'); return; } if (password !== confirmPassword) { setStatus('The passwords do not match.', 'error'); return; } submitButton.disabled = true; setStatus('Creating your member account…'); try { const result = await createUserWithEmailAndPassword(auth, email, password); await updateProfile(result.user, { displayName }); setStatus('Member account created. Opening your dashboard…'); } catch (error) { const messages = { 'auth/email-already-in-use': 'An account with this email already exists. Please sign in instead.', 'auth/invalid-email': 'Please enter a valid email address.', 'auth/weak-password': 'Please choose a stronger password.' }; setStatus(messages[error.code] || 'Unable to create the account. Please try again.', 'error'); submitButton.disabled = false; } });
...
const renderSignedInNavigation = (role) => { const desktopNav = document.querySelector('.desktop-nav'); const mobileNav = document.querySelector('.mobile-nav'); const adminLink = ['admin', 'superadmin'].includes(role) ? '<a href="admin.html">Admin Dashboard</a>' : ''; const html = `<a href="dashboard.html">Dashboard</a><a href="community.html">The V2 Room</a><a href="business-profile.html">Business Profile</a><a href="business-tools.html">Business Tools</a>${adminLink}<a href="index.html">Public Site</a><button class="nav-sign-out" type="button">Sign Out</button>`; [desktopNav, mobileNav].forEach((nav) => { if (!nav) return; nav.innerHTML = html; nav.querySelector('.nav-sign-out')?.addEventListener('click', async () => { await signOut(auth); window.location.replace('index.html'); }); }); };
...
onAuthStateChanged(auth, async (user) => {
  if (!user) { renderSignedOutNavigation(); setRoleUI('member', 'free', 'active'); if (isMemberPage || isAdminPage) window.location.replace('auth.html'); return; }
  ...
  setRoleUI(profile.role, profile.plan, profile.accountStatus);
